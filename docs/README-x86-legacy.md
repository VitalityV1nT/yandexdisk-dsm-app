> ⚠️ **АРХИВ — документация исходной x86-версии.** Этот файл описывает прежнюю
> сборку на проприетарном демоне `yandex-disk` для процессоров **x86_64** и
> сохранён только для истории. Он **не относится** к ARM-сборке (DS124 и др.),
> которую распространяет этот репозиторий. Актуальная инструкция (движок `rclone`,
> OAuth, расписание, сборка пакета) — в главном [README.md](../README.md).
>
> Команды и диалоги ниже (вопрос про прокси, «Starting daemon process…Done»,
> ручная сборка `tar … YandexDisk.spk`) относятся к x86-демону и на ARM-сборке
> работать не будут.

---

# Yandex Disk UI для Synology DSM (x86, архив)
